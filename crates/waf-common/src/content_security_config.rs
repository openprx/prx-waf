...
pub struct ContentSecurityConfig {
    ...
    /// `"nosql_injection"` / `"ssti"` / `"ldap_injection"`). An absent or disabled
    /// family contributes nothing (plan §6.2).
    ...
    /// `"nosql_injection"` / `"ssti"` / `"ldap_injection"`); each value
            ...
            if !is_known_attack_family(name) {
                return Err(format!(
                    "content_security.attacks has unknown family '{name}' \
                     (expected sql_injection/rce/xss/traversal/xxe/nosql_injection/ssti/ldap_injection)"
                ));
            }
...
fn is_known_attack_family(name: &str) -> bool {
    matches!(
        name,
        "sql_injection" | "rce" | "xss" | "traversal" | "xxe" | "nosql_injection" | "ssti" | "ldap_injection"
    )
}
