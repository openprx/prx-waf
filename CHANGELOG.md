...
Entries for the next release are not written here. Each one is a separate
file in [`changelog.d/`](changelog.d/), and `scripts/changelog.py release
<version>` assembles them into a version section below. This section is a
shared write target — every branch in flight appends to it — and it produced
a merge conflict that was committed to `main` with its markers intact. See
